...
[createCommandFunction](#createCommandFunction) Create a Lambda function to run a CLI command
...
Run a CLI command deployed with the
[createCommandFunction](#createCommandFunction) action. This action invokes
the Lambda function named `functionName`, loads the files with the specified
`urls` into a temporary folder and runs the specified CLI `command`.

The standard output from the CLI command is collected and returned to the
user. At this point, the action does not support collection of output files
generated by the CLI command.

Note that the executable name in `command` must match the filename in the
executable S3 URL passed to [createCommandFunction](#createCommandFunction).
...
##### <a name="createCommandFunction"></a>Create Command Function action
...
The action packs the code and configuration required to run the command line
